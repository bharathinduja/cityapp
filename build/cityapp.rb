...
helpers do
  ...
  def dbusername
    ENV['DBUsername'] or conjur_api.resource(ENV['DBUsername_CONJUR_VAR']).value or raise "Error no password or password conjur variable defined"
  end

  def dbpassword
    ENV['DBPassword'] or conjur_api.resource(ENV['DBPassword_CONJUR_VAR']).value or raise "Error no password or password conjur variable defined"
  end
